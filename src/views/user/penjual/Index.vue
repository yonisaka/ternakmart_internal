...
import { FETCH_PENJUAL } from "@/store/actions.type";
...
export default{
    ...
    beforeRouteEnter(to, from, next) {
        Promise.all([
        store.dispatch(FETCH_PENJUAL, to.params.slug),
        ]).then(() => {
        next();
        });
    },
    ...
    methods: {
        ...
        deleteItemConfirm () {
            const penjual_delete = axios.delete("/penjual/" + this.id_penjual)
            const user_delete = axios.delete("/users/" + this.id_user)
            axios.all([penjual_delete, user_delete])
            .then(() => {
                this.dialogDelete = false
                this.$router.go()
            })
